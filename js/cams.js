...
Cam.prototype.getIcon = function() {
	...
	var icon = {
	    ...
	    scale: iconScale,
	    strokeColor: (this.placement == place_OUT) ? '#c0392b' : '#8e44ad',
	    rotation: this.direction
	};
	return icon;
}

Cam.prototype.getPrettyType = function() {
	if (this.type == type_STATIC) {
		return "Stationary";
	}
	if (this.type == type_PTZ) {
		return "Pan-Tilt-Zoom";
	}
}

Cam.prototype.getPrettyPlacement = function() {
	if (this.placement == place_IN) {
		return "Indoors";
	}
	if (this.placement == place_OUT) {
		return "Outdoors";
	}
}
